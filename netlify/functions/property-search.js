...
exports.handler = async function(event) {
  ...
  // REAL lot size from Shape area (sq meters → sq feet) if available
  let lotSqft = 0;
  if(r['Shape.STArea()']) {
    lotSqft = Math.round(parseFloat(r['Shape.STArea()']) * 10.7639); // m² to ft²
  }
  if(!lotSqft || lotSqft < 1000 || lotSqft > 100000) {
    // Fallback to FAR estimate
