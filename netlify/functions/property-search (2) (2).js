...
function parseAddress(input) {
  ...
  // Strip common LA-area city names even when typed WITHOUT a comma
  // (e.g. "5439 geer street los angeles 90016"), so the city doesn't get
  // swallowed into the street name and break matching.
  const CITY_NAMES = ['LOS ANGELES','NORTH HOLLYWOOD','VAN NUYS','SHERMAN OAKS','STUDIO CITY',
    'WOODLAND HILLS','CANOGA PARK','GRANADA HILLS','PORTER RANCH','SUN VALLEY','VALLEY VILLAGE',
    'WEST HILLS','CULVER CITY','MAR VISTA','EAGLE ROCK','HIGHLAND PARK','SILVER LAKE','VENICE',
    'ENCINO','TARZANA','RESEDA','NORTHRIDGE','CHATSWORTH','WINNETKA','ARLETA','SYLMAR','PACOIMA',
    'PANORAMA CITY','MISSION HILLS','SAN FERNANDO','GLENDALE','BURBANK','PASADENA','WEST ADAMS',
    'BALDWIN HILLS','HYDE PARK','KOREATOWN','HOLLYWOOD','LOS FELIZ','ATWATER VILLAGE','PALMS'];
  for(const c of CITY_NAMES) {
    streetPart = streetPart.replace(new RegExp('\\b' + c + '\\b', 'g'), '').trim();
  }
  streetPart = streetPart.replace(/\s{2,}/g, ' ').replace(/,\s*$/, '').trim();
...
async function findParcel(parsed) {
  ...
  // Core street name = first significant word, with any street suffix stripped.
  // The county may store a parcel as AVE while a user (or MLS) types ST, so we
  // deliberately match on the bare name (e.g. "GEER") and never on the suffix.
  const SUFFIXES = new Set(['ST','AVE','BLVD','DR','RD','PL','LN','CT','TER','CIR','WAY','PKWY','HWY','ST.','AVENUE','STREET']);
  const words = (parsed.streetName || '').split(/\s+/).filter(Boolean);
  const DIRS = new Set(['N','S','E','W','NE','NW','SE','SW','NORTH','SOUTH','EAST','WEST']);
  // Drop leading direction and trailing suffix to isolate the real name.
  const coreWords = words.filter(w => !SUFFIXES.has(w) && !DIRS.has(w));
  const streetCore = coreWords[0] || words[0] || parsed.streetName;

  const queries = [];
  if(parsed.houseNo && streetCore) {
    // Most specific first: house# + core name + ZIP
    if(parsed.zip) {
      queries.push(`SitusHouseNo='${esc(parsed.houseNo)}' AND UPPER(SitusStreet) LIKE '%${esc(streetCore)}%' AND SitusZIP LIKE '${esc(parsed.zip)}%'`);
    }
    // house# + core name (any suffix, any ZIP)
    queries.push(`SitusHouseNo='${esc(parsed.houseNo)}' AND UPPER(SitusStreet) LIKE '%${esc(streetCore)}%'`);
    // house# + ZIP only — catches street-name/suffix mismatches entirely
    if(parsed.zip) {
      queries.push(`SitusHouseNo='${esc(parsed.houseNo)}' AND SitusZIP LIKE '${esc(parsed.zip)}%'`);
    }
    // full-address contains house# and core name
    queries.push(`UPPER(SitusFullAddress) LIKE '%${esc(parsed.houseNo)}%${esc(streetCore)}%'`);
  } else if(parsed.houseNo && parsed.zip) {
    queries.push(`SitusHouseNo='${esc(parsed.houseNo)}' AND SitusZIP LIKE '${esc(parsed.zip)}%'`);
  }
  ...
  for(const where of queries) {
    ...
    try {
      ...
      if(features.length > 0) {
        // If we have a core street name, prefer a result whose SitusStreet
        // actually contains it (guards the broad house#+ZIP fallback against
        // matching a same-numbered house on a different street).
        let pool = features;
        if(streetCore) {
          const named = features.filter(f => (f.attributes.SitusStreet || '').toUpperCase().includes(streetCore));
          if(named.length) pool = named;
        }
        // Prefer single-family residential within the chosen pool.
        const sfr = pool.find(f => ['0100','0101','0102','0103','010C','010D'].includes(f.attributes.UseCode));
        return { record: (sfr || pool[0]).attributes, matchedWhere: where };
      }
    } catch(e) {
