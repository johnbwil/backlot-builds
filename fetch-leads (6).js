...
exports.handler = async function(event) {
  ...
  for(const r of allRecords) {
    ...
    // Pool detection heuristic: estimate home value from sqft and ZIP medRent
    // Public LA County GIS does not expose assessed Roll values without auth
    const zd_early = ZIP_DATA[(r.SitusZIP||r._bzip||'').toString().trim().slice(0,5)] || { medRent: 2200 };
    const homeVal_est = zd_early.medRent * 380 * Math.max(0.7, Math.min(1.5, sqft/1400));
